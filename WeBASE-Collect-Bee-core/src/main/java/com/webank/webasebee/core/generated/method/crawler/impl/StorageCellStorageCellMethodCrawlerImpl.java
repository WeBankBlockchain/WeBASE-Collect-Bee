...
import com.webank.webasebee.core.generated.db.method.repository.StorageCellStorageCellMethodRepository;
...
@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.StorageCell.StorageCellMethodCrawlerService", havingValue = "on")
public class StorageCellStorageCellMethodCrawlerImpl implements BcosMethodCrawlerInterface {

	@Autowired
	private ContractMapsInfo contractMapsInfo; 
	@Autowired
	private StorageCellStorageCellMethodRepository repository;
	...
	public void transactionHandler(Transaction transaction, BigInteger blockTimeStamp, Map.Entry<String, String> entry, String methodName) {
		log.debug("Begin process StorageCellStorageCell Transaction");
		StorageCellStorageCell entity = new StorageCellStorageCell();
		...
		entity.setStorageHash(String.valueOf(params.get(0).getValue()));
		entity.setStorageInfo(String.valueOf(params.get(1).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));		
		repository.save(entity);
		
		log.debug("end process StorageCellStorageCell Transaction");
	}
}
